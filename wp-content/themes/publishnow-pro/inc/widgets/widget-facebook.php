...
class PublishnowPro_Facebook_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'facebook-likebox',
			apply_filters( 'publishnow_widget_name', esc_html__( '&raquo; Facebook Like Box', 'publishnow' ) ),
			array(
				'classname' => 'widget_facebook_likebox',
				'description' => esc_html__( 'Display a Facebook Like Box to connect visitors to your Facebook Page', 'publishnow' )
			)
		);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( empty( $like_args['href'] ) || ! $this->is_valid_facebook_url( $like_args['href'] ) ) {
			if ( current_user_can('edit_theme_options') ) {
				echo $before_widget;
				echo '<p>' . sprintf( esc_html__( 'It looks like your Facebook URL is incorrectly configured. Please check it in your <a href="%s">widget settings</a>.', 'publishnow' ), admin_url( 'widgets.php' ) ) . '</p>';
				echo $after_widget;
			}
		...
		$like_bg_colour          = apply_filters( 'publishnow_fb_likebox_bg', ( 'dark' == $like_args['colorscheme'] ? '#000' : '#fff' ), $like_args['colorscheme'] );
		...
		if ( ! empty( $title ) ) :
			...
			echo apply_filters( 'publishnow_facebook_likebox_title', $likebox_widget_title, $title, $page_url );
		...
		do_action( 'publishnow_stats_extra', 'widget', 'facebook-likebox' );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'href' ); ?>">
				<?php esc_html_e( 'Facebook Page URL', 'publishnow' ); ?>
				...
				<br />
				<small><?php esc_html_e( 'The Like Box only works with <a href="http://www.facebook.com/help/?faq=174987089221178">Facebook Pages</a>.', 'publishnow' ); ?></small>
			</label>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'width' ); ?>">
				<?php esc_html_e( 'Width', 'publishnow' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'height' ); ?>">
				<?php esc_html_e( 'Height', 'publishnow' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'colorscheme' ); ?>">
				<?php esc_html_e( 'Color Scheme', 'publishnow' ); ?>
				<select name="<?php echo $this->get_field_name( 'colorscheme' ); ?>" id="<?php echo $this->get_field_id( 'colorscheme' ); ?>">
					<option value="light" <?php selected( $like_args['colorscheme'], 'light' ); ?>><?php esc_html_e( 'Light', 'publishnow' ); ?></option>
					<option value="dark" <?php selected( $like_args['colorscheme'], 'dark' ); ?>><?php esc_html_e( 'Dark', 'publishnow' ); ?></option>
				</select>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_faces' ); ?>">
				...
				<?php esc_html_e( 'Show Faces', 'publishnow' ); ?>
				<br />
				<small><?php esc_html_e( 'Show profile photos in the plugin.', 'publishnow' ); ?></small>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'stream' ); ?>">
				...
				<?php esc_html_e( 'Show Stream', 'publishnow' ); ?>
				<br />
				<small><?php esc_html_e( 'Show the profile stream for the public profile.', 'publishnow' ); ?></small>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_border' ); ?>">
				...
				<?php esc_html_e( 'Show Border', 'publishnow' ); ?>
				<br />
				<small><?php esc_html_e( 'Show a border around the plugin.', 'publishnow' ); ?></small>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'force_wall' ); ?>">
				...
				<?php esc_html_e( 'Show Wall', 'publishnow' ); ?>
				<br />
				<small><?php esc_html_e( 'Show the wall for a Places page rather than friend activity.', 'publishnow' ); ?></small>
			</label>
		</p>
	...
	function get_default_args() {
		...
		return apply_filters( 'publishnow_facebook_likebox_defaults', $defaults );
	}
